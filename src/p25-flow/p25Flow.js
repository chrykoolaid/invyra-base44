import { P25_INV, P25_INV_FIELDS, P25_INV_FIXTURES, P25_INV_STATUS, P25_INV_STEPS } from './p25Fixtures.js';
...
function fieldsPresent(descriptor) {
  return P25_INV_FIELDS.every((field) => {
...
function expectedOrder(steps) {
  return steps.length === P25_INV_STEPS.length && steps.every((step, index) => step === P25_INV_STEPS[index]);
}
...
export function buildP25InventoryFlow(input = {}) {
  ...
  return Object.freeze({
    phase: P25_INV,
    flow_id: descriptor.flow_id || 'inventory-p25-unidentified',
    environment,
    status: flowCandidate ? P25_INV_STATUS.CANDIDATE : P25_INV_STATUS.BLOCKED,
...
export function projectP25InventoryFlowResult(fixture) {
  const flow = buildP25InventoryFlow({ descriptor: fixture.descriptor });
...
export function getP25InventoryFlowResults(fixtures = P25_INV_FIXTURES) {
  return Object.freeze(fixtures.map(projectP25InventoryFlowResult));
}
